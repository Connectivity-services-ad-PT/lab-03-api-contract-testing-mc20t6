...
- [x] Có test min/max hoặc dữ liệu sát ngưỡng (ví dụ nhiệt độ 80 và 81).
- [x] Có test limit/pagination nếu endpoint có danh sách (ví dụ limit=101).
- [x] Có test payload lớn hoặc metadata thiếu.
- [x] Có ghi chú kỳ vọng xử lý dữ liệu biên.

## 5. Reliability tests cơ bản

- [x] Có kiểm tra response time (đặc biệt khi chạy local service).
- [x] Có mô tả timeout mong muốn.
- [x] Có test hoặc ghi chú retry/idempotency nếu phù hợp (có Header X-Trace-Id).
- [x] Có consumer-side smoke test với ít nhất 1 mock của nhóm khác (mock AI Vision).
